...
from view.utils.utils_bbox import dist2bbox, make_anchors
...
class Loss:
    def __init__(self, model):
        ...
        self.stride = model.stride  # model_data strides
...
def is_parallel(model):
    # Returns True if model_data is of type DP or DDP
    return type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)


def de_parallel(model):
    # De-parallelize a model_data: returns single-GPU model_data if model_data is of type DP or DDP
...
class ModelEMA:
    ...
    Keeps a moving average of everything in the model_data state_dict (parameters and buffers)
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # if next(model_data.parameters()).device.type != 'cpu':
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            msd = de_parallel(model).state_dict()  # model_data state_dict
